...
class Matrix():
    ...
    def __add__(self, b):                                                   #edits what happens when two matricies are added
        if isinstance(b, Matrix):
            ...
            for row in range(self.rows):                                          #sets each value in the matrix equal to the sum of its respective counterparts in the other two matrices
                for column in range(self.columns):
                    result.matrix[row][column] = self.matrix[row][column] + b.matrix[row][column]
        else:
